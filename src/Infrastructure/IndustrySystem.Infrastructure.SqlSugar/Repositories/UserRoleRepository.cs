	...
	public async Task SetUserRolesAsync(Guid userId, IEnumerable<Guid> roleIds, CancellationToken ct = default)
	{
		...
		try
		{
			...
			if (roleIds != null)
			{
				...
				var records = roleIds.Distinct()
					.Select(rid => new UserRole { UserId = userId, RoleId = rid, CreatedAt = now })
					.ToList();
