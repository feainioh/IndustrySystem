...
public class LoginViewModel : BindableBase
{
 ...
 { _auth = auth; _state = state; LoginCommand = new DelegateCommand(async () => await LoginAsync(), CanLogin).ObservesProperty(() => UserName).ObservesProperty(() => Password);
 ...
 {
 ErrorMessage = null; IsBusy = true; ((DelegateCommand)LoginCommand).RaiseCanExecuteChanged();
 try
 {
 if (await _auth.SignInAsync(UserName, Password))
 {
 if (Remember) SaveRemembered(); else ClearRemembered();
 _state.SetAuthenticated(UserName);
 }
 else
 {
 ErrorMessage = "µÇÂ¼Ê§°Ü£¬Çë¼ì²éÕËºÅ»òÃÜÂë";
 }
 }
 finally
 {
 IsBusy = false; ((DelegateCommand)LoginCommand).RaiseCanExecuteChanged();
 }
 }
 ...
 {
 _state.SetAuthenticated(u);
 }
 else
 {
 ErrorMessage = "µÇÂ¼Ê§°Ü";
 }
 }
 finally
 {
 ...
 private void Cancel() { UserName = string.Empty; Password = string.Empty; ErrorMessage = null; }
