...
public partial class SamplingParameterEditDialog : UserControl
{
    ...
    public SamplingParameterEditDialog(IExperimentParameterAppService svc, ExperimentTemplateViewModel parentVm)
    {
        InitializeComponent();
        _vm = new ExperimentParameterEditorViewModel(svc, ExperimentType.Sampling);
        DataContext = _vm;
        _parentVm = parentVm;
        _parentVm.PropertyChanged += OnParentPropertyChanged;
        if (_parentVm.CurrentParameterDetail is { } detail && detail.Type == ExperimentType.Sampling)
    ...
    private void OnParentPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(ExperimentTemplateViewModel.CurrentParameterDetail))
        {
            ...
            if (detail is not null && detail.Type == ExperimentType.Sampling)
                _vm.LoadParameterDetail(detail);
        }
    }
}
