...
public partial class CustomDetectionParameterEditDialog : UserControl
{
    private readonly ExperimentParameterEditorViewModel _vm;
    private readonly ExperimentTemplateViewModel? _parentVm;

    public CustomDetectionParameterEditDialog(IExperimentParameterAppService svc, ExperimentTemplateViewModel parentVm)
    {
        InitializeComponent();
        _vm = new ExperimentParameterEditorViewModel(svc, ExperimentType.CustomDetection);
        DataContext = _vm;
        _parentVm = parentVm;
        _parentVm.PropertyChanged += OnParentPropertyChanged;
        if (_parentVm.CurrentParameterDetail is { } detail && detail.Type == ExperimentType.CustomDetection)
    ...
    private void OnParentPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(ExperimentTemplateViewModel.CurrentParameterDetail))
        {
            ...
            if (detail is not null && detail.Type == ExperimentType.CustomDetection)
                _vm.LoadParameterDetail(detail);
        }
    }
}
