...
public partial class RotaryEvaporationParameterEditDialog : UserControl
{
    private readonly ExperimentParameterEditorViewModel _vm;
    private readonly ExperimentTemplateViewModel? _parentVm;

    public RotaryEvaporationParameterEditDialog(IExperimentParameterAppService svc, ExperimentTemplateViewModel parentVm)
    {
        InitializeComponent();
        _vm = new ExperimentParameterEditorViewModel(svc, ExperimentType.RotaryEvaporation);
        DataContext = _vm;
        _parentVm = parentVm;
        _parentVm.PropertyChanged += OnParentPropertyChanged;
        if (_parentVm.CurrentParameterDetail is { } detail && detail.Type == ExperimentType.RotaryEvaporation)
    ...
    private void OnParentPropertyChanged(object? sender, System.ComponentModel.PropertyChangedEventArgs e)
    {
        if (e.PropertyName == nameof(ExperimentTemplateViewModel.CurrentParameterDetail))
        {
            ...
            if (detail is not null && detail.Type == ExperimentType.RotaryEvaporation)
                _vm.LoadParameterDetail(detail);
        }
    }
}
